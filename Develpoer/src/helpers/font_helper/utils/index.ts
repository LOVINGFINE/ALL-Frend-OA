...
export default async (options: SvgToFontOptions = {}) => {
  ...
  try {
    ...
    if (options.css) {
      const styleTemplatePath =
        ...
        (!options.useNameAsUnicode
          ...
          : path.resolve(__dirname, "ligature-styles"));
    ...
    if (options.website) {
      ...
      const indexName = pageName.includes(options?.website?.index || "")
        ? pageName.indexOf(options?.website?.index || "")
        : 0;
      ...
      options.website.template =
        options.website.template ||
        path.join(__dirname, "website", "index.ejs");
      // template data
      ...
      if (
        ...
        fs.pathExistsSync(options.website.favicon)
      ) {
      ...
      const unicodeHtmlStr = await createHTML(
        options.website.template,
        tempData
      );
      ...
      const symbolHtmlStr = await createHTML(
        options.website.template,
        tempData
      );
